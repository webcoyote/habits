...
struct TimeDisplaySelectionView: View {
    @ObservedObject private var appSettings = AppSettings.shared
    
    var body: some View {
        SettingsSelectionView(
            title: "Time Display",
            options: TimeDisplayMode.allCases,
            currentSelection: appSettings.timeDisplayMode,
            onSelection: { mode in
                appSettings.timeDisplayMode = mode
            },
            displayName: { $0.displayName }
        )
    }
...
struct WeightUnitsSelectionView: View {
    @ObservedObject private var appSettings = AppSettings.shared
    
    var body: some View {
        SettingsSelectionView(
            title: "Weight Units",
            options: WeightUnitsMode.allCases,
            currentSelection: appSettings.weightUnitsMode,
            onSelection: { mode in
                appSettings.weightUnitsMode = mode
            },
            displayName: { $0.displayName }
        )
    }
